...
def check_for_fixes(appid: int) -> str:
    ...
    # CUSTOM INTEGRATION: Local online_fix.txt bypass
    try:
        install_path_res = get_game_install_path_response(appid)
        if install_path_res.get("success") and install_path_res.get("installPath"):
            install_path = install_path_res["installPath"]
            override_file = os.path.join(install_path, "online_fix.txt")
            if os.path.exists(override_file):
                ...
                if url and url.startswith("http"):
                    ...
                    result["onlineFix"]["url"] = url
                    logger.log(f"LuaTools: Found online_fix.txt override for {appid}: {url}")
                    # Skip fetching index when overridden
                    return json.dumps(result)
    except Exception as e:
        logger.warn(f"LuaTools: Failed reading online_fix.txt for {appid}: {e}")
...
def _download_and_extract_fix(appid: int, download_url: str, install_path: str, fix_type: str, game_name: str = ""):
    ...
    try:
        ...
        ext = "rar" if download_url.lower().endswith(".rar") else "zip"
        dest_zip = os.path.join(dest_root, f"fix_{appid}.{ext}")
        ...
        extracted_files = []
        if dest_zip.endswith(".rar"):
            logger.log(f"LuaTools: Extracting RAR archive using 7-Zip")
            import subprocess
            seven_z_path = r"C:\Program Files (x86)\Steam\plugins\luatools\7z\7z.exe"
            if not os.path.exists(seven_z_path):
                raise Exception("7z.exe not found! Please contact developer.")
            pwd = "online-fix.me" if "online-fix.me" in download_url else ""
            cmd = [seven_z_path, "x", f"-p{pwd}", "-y", dest_zip, f"-o{install_path}"]
            res = subprocess.run(cmd, capture_output=True, text=True)
            if res.returncode != 0:
                logger.warn(f"LuaTools: 7z.exe extraction failed: {res.stderr}\n{res.stdout}")
                raise Exception("RAR Extraction failed (Password missing or bad archive)")
            
            list_cmd = [seven_z_path, "l", f"-p{pwd}", dest_zip]
            ...
            for line in list_res.stdout.splitlines():
                if "-------------------" in line:
                    ...
                    continue
                if scanning and len(line) > 53:
                    name = line[53:].strip()
                    if not name.endswith("\\") and not name.endswith("/"):
                        extracted_files.append(name.replace("\\", "/"))
        else:
